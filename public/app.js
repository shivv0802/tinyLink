async function fetchLinks(){return (await fetch('/api/links')).json();}
...
document.getElementById('createForm').addEventListener('submit',async e=>{
 ...
 const res=await fetch('/api/links',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({target_url:url,code:code||undefined})});
 const data=await res.json();
 if(!res.ok){msg.textContent=data.error;return;}
 msg.textContent='Created!'; msg.style.color='green';
 document.getElementById('target_url').value=''; document.getElementById('code').value='';
 refresh();
});
...
document.querySelector('#linksTable tbody').addEventListener('click',async e=>{
 ...
 if(e.target.classList.contains('copy')){
   ...
   e.target.textContent='Copied!'; setTimeout(()=>e.target.textContent='Copy',900);
 }
   ...
   await fetch('/api/links/'+code,{method:'DELETE'}); refresh();
 }
});
